...
class FlaxDDPMScheduler(FlaxSchedulerMixin, ConfigMixin):
    ...
    def _get_variance(self, state: DDPMSchedulerState, t, predicted_variance=None, variance_type=None):
        prev_t = t - self.config.num_train_timesteps // 15
...
#         elif variance_type == "learned_range":
    ...
    def step(
        ...
        prev_t = timestep - self.config.num_train_timesteps // 15
        ...
        def random_variance():
            ...
            save_(variance,'small_v_'+str(self.current_step)+'.npy')
            variance = jnp.exp(0.5 * variance) * noise
            save_(predicted_variance,'predicted_v_'+str(self.current_step)+'.npy')

            return variance#(self._get_variance(state, t, predicted_variance=predicted_variance) ** 0.5) * noise
