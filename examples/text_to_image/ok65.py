...
#     )
    ...
    def check_str2(path):
      for s in path:
        if 'atte' in s and 'attn2' not in s:# or 'up_blocks_3' in s or 'norm' in s or 'bias' in s or 'emb' in s.lower() or 'conv_in' in s or 'conv_out' in s:
            print("success ---> " , path )
            return True
#       print("fail ----> ", path )      
      return False
...
#     train_rngs = jax.random.split(rng, jax.local_device_count())
    ...
    p_train_step = pjit(
        train_step,
        in_axis_resources=( unet_param_spec,unet_opt_state_spec,text_param_spec,vae_param_spec,P("dp",None),P(None,None,'dp','mp'),None ),
