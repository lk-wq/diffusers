    ...
    def partition_shape(shape):
      ...
      if len(shape) == 1:
        if shape[0] % 4 == 0:
          ...
          return P("mp")
      if len(shape) == 2:
        if shape[0] % 4 == 0 and shape[1] % 2 == 0 if shape[0] > shape[1]:
          return P("dp","mp")
...
#     grad_fun = jax.jit(jax.value_and_grad(loss_jit))
    apply_fn_text = jax.jit(
    text_encoder.__call__
    )
    apply_fn_unet = jax.jit(
    unet.apply
    )
...
#                 text_updates , text_opt_state = optimizer.update( grads , text_opt_state , text_params )
            ...
            noise_rng, timestep_rng = jax.random.split(train_rngs)
            noise = jax.random.normal(noise_rng, latents.shape)
            # Sample a random timestep for each image
            bsz = latents.shape[0]
            timesteps = jax.random.randint(
...
#             save_(unet_params['time_embedding']['linear_1']['kernel'],'k3_post.npy')
    #         print("unet params post ---->" ,unet_params['time_embedding']['linear_1']['kernel'])
            unet_outputs = apply_fn_unet({"params": unet_params},noisy_latents, timesteps, encoder_hidden_states, train=False)
